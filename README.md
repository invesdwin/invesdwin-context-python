...
```
- **invesdwin-context-python-runtime-python4j**: This library integrates [python4j](https://deeplearning4j.konduit.ai/python4j/tutorials/quickstart). It uses an embedded CPython distribution via [JavaCPP](https://github.com/bytedeco/javacpp-presets/tree/master/cpython). It only supports CPython 3.x and does not provide sandboxed interpreters. The benefit of this integration is no python installation is required on the host system since it brings its own.
